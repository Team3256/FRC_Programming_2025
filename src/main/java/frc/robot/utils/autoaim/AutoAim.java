...
public class AutoAim {
    ...
    public static final Translation2d BLUE_REEF_CENTER = new Translation2d(Units.inchesToMeters(176.746),
    ...
    public static Command translateToPose(CommandSwerveDrivetrain swerve, Supplier<Pose2d> target) {
        ...
        final ProfiledPIDController vxController = new ProfiledPIDController(
                100,
                0.01,
                0.0,
                new TrapezoidProfile.Constraints(MAX_AUTOAIM_SPEED, MAX_AUTOAIM_ACCELERATION));
        final ProfiledPIDController vyController = new ProfiledPIDController(
                100,
                0.01,
                0.0,
                new TrapezoidProfile.Constraints(MAX_AUTOAIM_SPEED, MAX_AUTOAIM_ACCELERATION));
        ...
        return Commands.runOnce(
                () -> {
                    ...
                    vxController.reset(
                            swervePose.getX(),
                            swerve.getVelocityFieldRelative().vxMetersPerSecond * Math.signum(diff.getX()) < 0.0
                                    ? swerve.getVelocityFieldRelative().vxMetersPerSecond
                                    : 0.0);
                    vyController.reset(
                            swervePose.getY(),
                            swerve.getVelocityFieldRelative().vyMetersPerSecond * Math.signum(diff.getY()) < 0.0
                ...
                .andThen(
                        swerve.driveVelocityFieldRelative(
                                () -> {
                                    var swervePose = swerve.getState().Pose;
                                    final var diff = swervePose.minus(cachedTarget[0]);
                                    final var speeds = MathUtil.isNear(0.0, diff.getX(), Units.inchesToMeters(0.25))
                                            && MathUtil.isNear(0.0, diff.getY(), Units.inchesToMeters(0.25))
                                            && MathUtil.isNear(0.0, diff.getRotation().getDegrees(), 0.5)
                                                    ? new ChassisSpeeds()
                                                    : new ChassisSpeeds(
                                                            vxController.calculate(swervePose.getX(),
                                                                    cachedTarget[0].getX())
                                                                    + vxController.getSetpoint().velocity,
                                                            vyController.calculate(swervePose.getY(),
                                                                    cachedTarget[0].getY())
    ...
    /*
     * 8033 exclusively uses this to score in Barge. Commented out because may be
     * useful later (?)
     */
    ...
    // 0.0,
    // 0.0,
    // new TrapezoidProfile.Constraints(MAX_ANGULAR_SPEED,
    ...
    // 0.0,
    // 0.0,
    // new TrapezoidProfile.Constraints(MAX_AUTOAIM_SPEED,
    // MAX_AUTOAIM_ACCELERATION));
    // return Commands.runOnce(
    // () -> {
