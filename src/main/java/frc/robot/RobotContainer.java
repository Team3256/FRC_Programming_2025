...
public class RobotContainer {
  ...
  private final AdaptiveSlewRateLimiter swerveVelXRateLimiter = new AdaptiveSlewRateLimiter(0.5, 0.5);
  private final AdaptiveSlewRateLimiter swerveVelYRateLimiter = new AdaptiveSlewRateLimiter(0.5, 0.5);
  private final AdaptiveSlewRateLimiter swerveAngVelRateLimiter = new AdaptiveSlewRateLimiter(0.5, 0.5);
  ...
  private void configureSwerve() {
    ...
    if (FeatureFlags.kSwerveAccelerationLimitingEnabled) {
      drivetrain.applyRequest(
          () -> drive
              .withVelocityX(swerveVelXRateLimiter.calculate(m_driverController.getLeftY() * MaxSpeed)) // Drive -y is
                                                                                                        // forward
              .withVelocityY(swerveVelYRateLimiter.calculate(m_driverController.getLeftX() * MaxSpeed)) // Drive -x is
                                                                                                        // left
              .withRotationalRate(swerveAngVelRateLimiter.calculate(-m_driverController.getRightX() * MaxAngularRate)));
    } else {
      drivetrain.applyRequest(
          () -> drive
