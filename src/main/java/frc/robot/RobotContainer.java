...
public class RobotContainer {
    ...
    private final Elevator elevator = new Elevator(true,
            Utils.isSimulation() ? new ElevatorIOSim() : new ElevatorIOTalonFX());
                        ...
                        Commands.parallel( // Both run AutoAlign & check tolerance
                                ...
                                Commands.waitUntil(
                                        () -> AutoAim.isInToleranceCoral(
                                                drivetrain.getState().Pose)) // Additionally, once we're in tolerance,
                                                                             // rumble
                                        // controller
                                        .andThen(
                                                () -> {
                                                    m_driverController.setRumble(GenericHID.RumbleType.kBothRumble,
                                                            0.5);
                                                })
                                        .andThen(
                                                () -> {
