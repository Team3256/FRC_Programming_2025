...
public class NodeManager {
  ...
  public AutoRoutine createAuto(ArrayList<Node> nodes, Superstructure superstructure) {
    ...
    for (Node node : nodes) {
      ...
      switch (node.nodeType()) {
        case PRELOAD -> {
          ...
          nextTrajTrigger.toggleOnTrue(
              preloadTraj
                  .resetOdometry()
                  .andThen(preloadTraj.cmd())
                  // TODO: tune manipulator side
                  .andThen(superstructure.scoreCoral(node.scoringType(), ManipulatorSide.LEFT)));
